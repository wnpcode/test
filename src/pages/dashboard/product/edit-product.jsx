...
const EditProduct = () => {
  ...
  const getData = async () => {
    try {
      ...
      let response = await axiosGet(
        `${URL_CONSTANT.product.all[getCurrentRole()]}/${params.id}`
      );
      ...
      console.log(response.data.images, "image from server");
      _.map(response.data.images, (image, index) => {
        files[`file${index + 1}`] = image;
      });
      setFileShow(files);
      setFileName({
        file1: "",
  ...
  const changeFile = async (file, key) => {
    ...
    try {
      ...
      let response = await axiosPost(
        `${URL_CONSTANT.product.image[getCurrentRole()]}`,
        formData
      );
      getData();
      setFileShow({
  ...
  const deleteImage = async (key) => {
    ...
    try {
      loading.setLoading(true);
      let response = await axiosDeleteWithBody(
        `${URL_CONSTANT.product.image[getCurrentRole()]}`,
        newData
      );
      setFileShow({
